...
Page({
  ...
  getUserList: function(record){
    console.log(record)
    let playerIdsSet = new Set();
    Object.keys(record).forEach(key => {
      if(Array.isArray(record[key])){
        record[key].forEach(item => {
          if (item.player_ids) {
            item.player_ids.split(',').forEach(id => {
              playerIdsSet.add(id.trim());
            });
          }
        });
      }
    });
  // 将Set转换为数组，然后使用join(',')方法将数组转换为用逗号连接的字符串
    let uniquePlayerIdsString = Array.from(playerIdsSet).join(',');
    return uniquePlayerIdsString
  },
  onLoad: function (options) {
    ...
    wx.cloud.callFunction({
      name: 'requestGEM',
      data: {
        url: "http://gemtd.ppbizon.com/gemtd/leaderboard/@0"
      },
      success: res => {
        const record = res.result.data;
        const userList = this.getUserList(record) 
        wx.cloud.callFunction({
          name: 'requestGEM',
          data: {
            url: "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=0A73E70ED398C36AD29C0AB210F71B3D&steamids=" + userList
          },
          success: res => {
            let players = res.result.response.players
            let user = {}
            players.forEach(element => {
              user[element.steamid] = {"avatar":element.avatarmedium}
            });
            record.user = user
          ...
          fail: res => {
            const recordStorage = wx.getStorageSync('record');
            if(recordStorage){
              this.setData({
                record: record
              })
        ...
        this.getRankList(this.data.pickerIndex + 1);
        wx.hideLoading();
      }
    });
  },
  getRankList: function (index) {
    ...
    const data = this.data.record[selectP]
  ...
  parseAndTranslate: function (str) {
